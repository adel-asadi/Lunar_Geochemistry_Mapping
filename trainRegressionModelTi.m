function [trainedModelTi, TivalidationRMSE] = trainRegressionModelTi(trainingData)
...
% Auto-generated by MATLAB on 29-Sep-2019 20:42:10
...
trainedModelTi.predictFcn = @(x) gpPredictFcn(predictorExtractionFcn(x));

% Add additional fields to the result struct
trainedModelTi.RegressionGP = regressionGP;
trainedModelTi.About = 'This struct is a trained model exported from Regression Learner R2018a.';
trainedModelTi.HowToPredict = sprintf('To make predictions on a new predictor column matrix, X, use: \n  yfit = c.predictFcn(X) \nreplacing ''c'' with the name of the variable that is this struct, e.g. ''trainedModel''. \n \nX must contain exactly 5 columns because this model was trained using 5 predictors. \nX must contain only predictor columns in exactly the same order and format as your training \ndata. Do not include the response column or any columns you did not import into the app. \n \nFor more information, see <a href="matlab:helpview(fullfile(docroot, ''stats'', ''stats.map''), ''appregression_exportmodeltoworkspace'')">How to predict using an exported model</a>.');
...
cvp = cvpartition(size(response, 1), 'Holdout', 0.13);
...
TivalidationRMSE = sqrt(nansum(( validationPredictions - validationResponse ).^2) / numel(validationResponse(isNotMissing) ));
